...
object fp2 {
  ...
  def joinTerminateRight (xs : List[String], term : String) : String = {
    foldRight(xs, "", (q:String, r:String)=>q + term + r)
  }
  ...
  def joinTerminateLeft (xs : List[String], term : String) : String = {
    foldLeft(xs, "", (q: String, r: String) => q + r + term)
  }
  ...
  def firstIndexNumGreaterThan (a : Int, xs : List[Int]) : Int = {
    firstIndexNumGreaterThanAux(a, 0, xs)
  }
  def firstIndexNumGreaterThanAux (a : Int, i: Int, xs : List[Int]) : Int = {
    xs match{
      case Nil => throw new java.util.NoSuchElementException()
      case y::ys if (y>=a) => i
      case _::ys => firstIndexNumGreaterThanAux (a, i+1 ,ys)
    }
  }
}
